...
static CLAWR_HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
...
fn adaptive_default_theme_selection() -> (EmbeddedThemeName, &'static str) {
    match crate::v2::terminal_palette::default_bg() {
        Some(bg) if crate::v2::color::is_light(bg) => {
...
pub(crate) fn configured_theme_name() -> String {
    ...
    if let Some(Some(name)) = THEME_OVERRIDE.get() {
        ...
        if let Some(Some(home)) = CLAWR_HOME.get()
...
/// `.tmTheme` file under `{CLAWR_HOME}/themes/`.
